...
- **Market 枚举（2026-09）**：`pull.Market`（`type Market string`）+ 常量 `MarketAStock/MarketIndex/MarketEtfLof/MarketBlock/MarketFuture/MarketHK/MarketUS`，定义在 `extend/pull/market.go`。`Code.Market`、market 子包 Unit 的 `name` 字段均为该类型（`Name()` 返回 `.String()`）。**兼容性**：底层是 string，旧代码裸字符串字面量（如 `Market: "a_stock"`）因未类型化常量隐式转换仍可编译。注意与 `protocol.Exchange`（uint8 协议层市场编码）是两套体系。
- **PullConfig.Codes 扁平化 + ParseCode 自动路由（2026-09）**：`Codes` 从 `map[Market][]string` 改为扁平 `[]string`，NewService 一次性 `ParseCode` 解析并按市场分组（`Service.overrides`）。路由规则：带 sh/sz/bj 前缀 8 位→按谓词分类（ETF>股票>板块>指数）；6 位裸数字→`protocol.AddPrefix` 补前缀后分类（`000001`→股票 `sz000001` 非上证指数；北交所指数 899xxx 补不上前缀特判为 `bj+code`）；5 位数字→港股；纯字母→美股；含 `/`→期货；`market.code` 完整键也可。`protocol.IsBlock` 未导出，pull 包内 `isBlockCode` 复刻同规则（sh+880/881）。**破坏性变更**：`map[Market][]string` 旧写法编译失败，PullV2 示例已同步。
- **PullConfig.Units 已删除（2026-09，先增后删）**：`Units` 字段经历 `[]Unit`→`[]Market` 后最终**整个删除**——市场完全由 `Codes` 路由决定：`Codes` 空=全量模式（全部注册市场自动发现）；`Codes` 非空=白名单模式（只拉这些代码，拉取范围限定为涉及市场，其余市场不拉）。NewService 中 overrides 分组决定 units 集合。**测试适配**：mockUnit 注册名 "test"，测试用完整键 `"test.sh600000"` 走 ParseCode 路由（`market.code` 形式对任意注册市场可用）。
- **注册的内置 Unit**（`extend/pull/market/`，`init()` 中 `pull.Register`）：`a_stock`/`index`/`etf_lof`（标准行情7709，走 `Manage`）；`block`（板块指数，`GetBlockDataWithIndex`）；`future`/`hk`/`us`（扩展行情7727，走 `ExPool`）。`Codes` 空时全部拉取；白名单模式只拉涉及市场。
