...
从`https://github.com/jing2uo/tdx2db`复制而来
...
func Download(dir string) (string, error) {
	...
	zipFilename := filepath.Join(dir, "gbbq.zip")
...
func (this GBBQs) GetStocks() map[string]Equities {
	m := map[string]Equities{}
	for _, v := range this {
		switch v.Category {
		case 2, 3, 5, 7, 8, 9, 10:
			m[v.Code] = append(m[v.Code], &Equity{
				Category: v.Category,
				Code:     v.Code,
				Date:     v.Date,
				Float:    v.C3,
